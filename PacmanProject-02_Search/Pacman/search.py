...
def depthFirstSearch(problem):
    ...
    visited[start] = True
    step = []
    res = recurDFS(currStack, step, visited, problem )
    return res[1]
    util.raiseNotDefined()
...
def recurDFS(currStack, step, visited, problem ):
    for x in currStack:
        if x[_STATE] in visited:
            continue
        else:
            visited[x[_STATE]] = True
            step.append(x[_ACTION])
            if problem.isGoalState(x[_STATE]):
                return (True, step )
            else:
                res = recurDFS(problem.getSuccessors(x[_STATE]), step, visited, problem )
                if res[0] == True:
                    return res
                else:
                    step.pop()
                    del visited[x[_STATE]]
    return (False, [S] )
